...
export default function AdminVault() {
  ...
  const handleSeedPay = async () => {
    ...
    try {
      ...
      if (res?.success) {
        showToast(`✅ Seeded ${seedAmount} $PAY to ${seedUserId}`)
        setSeedUserId(''); setSeedAmount('')
        fetchPool()
      }
    } catch { showToast('❌ Failed to seed $PAY') }
    setSeeding(false)
  }
  ...
  const handleDistribute = async () => {
    ...
    try {
      ...
      fetchPool()
    } catch { showToast('❌ Distribution failed') }
  ...
  const handleAddRevenue = async () => {
    ...
    } catch { showToast('❌ Failed to add revenue') }
        ...
        <div style={{ display: 'grid', gap: 16 }}>
          ...
          <div style={S.card}>
            ...
            <div style={S.flexRow}>
              ...
              <button style={{ ...S.btn, background: '#16a34a', color: '#fff', marginTop: 18 }} onClick={handleAddRevenue} disabled={addingRevenue || !revAmount}>
          ...
          <div style={S.card}>
            ...
            <button style={{ ...S.btnPrimary, height: 44, fontSize: 14, padding: '0 24px' }} onClick={handleDistribute} disabled={distributing}>
              <i className="ti ti-vault" style={{ fontSize: 16 }} /> {distributing ? '⏳ Distributing...' : '📦 Run Distribution Now'}
            </button>
          </div>
