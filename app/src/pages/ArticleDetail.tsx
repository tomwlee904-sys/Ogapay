...
function renderContent(text: string) {
  ...
  for (const line of text.split('\n')) {
    if (line.startsWith('### ')) { html += `<h3>${escapeHtml(line.replace('### ', ''))}</h3>`; continue }
    if (line.startsWith('## '))  { html += `<h2>${escapeHtml(line.replace('## ', ''))}</h2>`; continue }
    if (line.startsWith('# '))   { html += `<h1>${escapeHtml(line.replace('# ', ''))}</h1>`; continue }
    if (line.startsWith('- '))   { html += `<li>${escapeHtml(line.replace('- ', ''))}</li>`; continue }
    if (line.startsWith('> '))   { html += `<blockquote>${escapeHtml(line.replace('> ', ''))}</blockquote>`; continue }
...
export default function ArticleDetail() {
  ...
  const badgeColors: Record<string, { bg: string; color: string }> = {
    News: { bg: '#E6F1FB', color: 'var(--accent)' },
    Businesses: { bg: '#EAF3DE', color: '#3B6D11' },
    Freelancers: { bg: '#EEEDFE', color: 'var(--accent)' },
    'Case Studies': { bg: '#FBEAF0', color: '#993556' },
  }
  const badge = badgeColors[post.category] || { bg: 'var(--accent)', color: 'var(--accent)' }
  ...
  return (
    <div data-theme={theme} style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', background: 'var(--bg)', color: 'var(--text)' }}>
      <style>{`
        ...
        .ad-content blockquote { border-left: 4px solid var(--accent); margin: 24px 0; padding: 12px 20px; background: rgba(var(--accent-rgb),0.04); border-radius: 0 8px 8px 0; font-style: italic; color: var(--text2); }
      ...
      <article style={{ maxWidth: 760, margin: '0 auto', padding: '40px 24px 60px', width: '100%', boxSizing: 'border-box' }}>
        <button onClick={() => navigate('/blog')} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 13, color: 'var(--accent)', fontWeight: 600, padding: 0, marginBottom: 24, display: 'flex', alignItems: 'center', gap: 4, fontFamily: 'inherit' }}>
          â† Back to blog
        </button>
        ...
        <div style={{ borderTop: '1px solid var(--border)', marginTop: 48, paddingTop: 24, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button onClick={toggleBookmark} style={{ height: 34, width: 34, padding: 0, borderRadius: 8, border: '1px solid var(--border)', background: 'var(--bg2)', color: bookmarked ? 'var(--accent)' : 'var(--text2)', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              {bookmarked ? (
                <svg width="16" height="16" viewBox="0 0 24 24" fill="var(--accent)" stroke="var(--accent)" strokeWidth="2"><path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"/></svg>
              ) : (
