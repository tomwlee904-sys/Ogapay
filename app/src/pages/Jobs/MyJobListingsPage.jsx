    ...
    <Layout>
      ...
      <div style={{ maxWidth: 800, margin: '0 auto', padding: '24px 16px 60px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 20 }}>
          ...
          <div style={{ background: C.card, border: '1px solid ' + C.border, borderRadius: 12, padding: '14px 16px', textAlign: 'center' }}>
            <div style={{ fontSize: 22, fontWeight: 900, color: '#16a34a' }}>{activeCount}</div>
              ...
              <div key={job.id} style={{ background: C.card, border: '1px solid ' + C.border, borderRadius: 12, padding: 16, display: 'flex', alignItems: 'center', gap: 14 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ display: 'flex', gap: 6, marginTop: 4, flexWrap: 'wrap' }}>
                    <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: 99, background: '#191C6B18', color: '#191C6B' }}>{job.job_type}</span>
                    <span style={{ fontSize: 10, fontWeight: 600, padding: '2px 8px', borderRadius: 99, background: job.status === 'active' ? 'rgba(22,163,74,0.12)' : C.bg2, color: job.status === 'active' ? '#16a34a' : C.text3 }}>
                      {job.status}
                    </span>
                  </div>
                </div>
                <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                  ...
                  <button onClick={() => handleToggleStatus(job)} disabled={togglingId === job.id}
                    style={{ padding: '8px 14px', borderRadius: 8, border: '1px solid ' + C.border, background: job.status === 'active' ? 'rgba(239,68,68,0.08)' : 'rgba(22,163,74,0.08)', color: job.status === 'active' ? '#ef4444' : '#16a34a', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>
