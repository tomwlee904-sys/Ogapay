...
const WORKSPACES: Record<string, { label: string; icon: string; desc: string; color: string }> = {
  ...
  development: { label: 'Dev Workspace', icon: 'code', desc: 'Software development, smart contracts, automation', color: 'var(--accent)' },
}
...
function formatReward(reward: number | string, currency?: string): string {
  ...
  if (cur === 'OGA' || cur === 'SOGA') return amount + ' $PAY'
  if (!['SOL', 'USDC', 'USDT', 'ETH', 'MATIC', 'BNB', 'BTC', 'NGN'].includes(cur)) {
    return `₦${amount.toLocaleString()}`
  }
  return formatCurrency(amount, cur as 'SOL' | 'USDC' | 'NGN')
}
    ...
    const fetchData = async () => {
      try {
        ...
        const [tasksRes, subRes] = await Promise.all([
          fetch(API_BASE + '/tasks?category=' + encodeURIComponent(apiCategory), { headers }).catch(() => null),
