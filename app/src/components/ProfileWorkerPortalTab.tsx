...
export default function TabWorkerPortalContent() {
  ...
  const workspaceItems = [
    ...
    { icon: 'ti ti-code', label: 'Dev', route: '/worker/development', color: 'var(--accent)' },
  ]
  ...
  const [stats, setStats] = useState([
    ...
    { icon: 'ti ti-gift', color: 'var(--accent)', count: 0, label: 'Tips Received' },
      ...
      <div className="wp-page">
        ...
        <div className="wp-stats-row">
          {stats.slice(0, 3).map((s, i) => (
            <span key={i}><i className={s.icon} style={{ color: s.color }} /> {s.count > 0 ? `${s.count} ${s.label}` : `No ${s.label.toLowerCase()} yet`}</span>
          ))}
        </div>
        ...
        <div className="wp-profile-card">
          <div className="wp-profile-top">
            <div className="wp-avatar-box">
              {authUser?.avatar ? <img src={authUser.avatar} alt="" style={{ width: '100%', height: '100%', borderRadius: 10, objectFit: 'cover' }} /> : <i className="ti ti-user" />}
            </div>
            <div className="wp-profile-info">
              <div className="wp-profile-name">
                {authUser?.firstName ? `${authUser.firstName} ${authUser.lastName || ''}` : 'No nickname yet'}
          ...
          <div className="wp-bio-box">
            ...
            <div className="wp-bio-text">{authUser?.bio || 'No bio yet. Add one to tell others about yourself!'}</div>
          </div>
        </div>
